...
from src.diagnostics.arbitration_gate_sweep_csv import (
    ARBITRATION_GATE_FIELDS,
    summarize_arbitration_gate_sweep,
)
...
STORE_PATH: Final[Path] = (
    Path("artifacts") / "arbitration_gate_sweep_memory.jsonl"
)
OUTPUT_PATH: Final[Path] = (
    Path("artifacts") / "arbitration_gate_sweep.csv"
)
...
def main() -> None:
    ...
    rows = run_arbitration_gate_sweep(DEFAULT_SEEDS)
    write_sweep_csv(
        rows=rows,
        output_path=OUTPUT_PATH,
    )

    print(f"Wrote arbitration gate sweep to {OUTPUT_PATH}")
    print(f"Arbitration gate memory store: {STORE_PATH}")
...
def run_arbitration_gate_sweep(
    seeds: Sequence[int],
...
def summarize_run(
    ...
    return tuple(
        ...
        for row in summarize_arbitration_gate_sweep(history)
    )
