...
from src.diagnostics.arbitration_readiness_csv import (
    ARBITRATION_READINESS_FIELDS,
    summarize_arbitration_readiness,
)
...
STORE_PATH: Final[Path] = (
    Path("artifacts") / "arbitration_readiness_memory.jsonl"
)
OUTPUT_PATH: Final[Path] = (
    Path("artifacts") / "arbitration_readiness_sweep.csv"
)
...
def main() -> None:
    ...
    rows = run_arbitration_readiness_sweep(DEFAULT_SEEDS)
    write_sweep_csv(
        rows=rows,
        output_path=OUTPUT_PATH,
    )

    print(f"Wrote arbitration readiness sweep to {OUTPUT_PATH}")
    print(f"Arbitration readiness memory store: {STORE_PATH}")
...
def run_arbitration_readiness_sweep(
    seeds: Sequence[int],
...
def summarize_run(
    ...
    return tuple(
        ...
        for row in summarize_arbitration_readiness(history)
    )
